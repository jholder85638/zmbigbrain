...
var SkipExcessiveZimbraTokens = false
// Parse wraps Unmarshal to parse an LDIF from a string
func Parse(str string, skipTokens bool) (l *LDIF, err error) {
	if(skipTokens){
		SkipExcessiveZimbraTokens = true
	}
...
func (l *LDIF) parseEntry(lines []string) (entry *Entry, err error) {
	...
	if !strings.HasPrefix(lines[0], "dn:") {
		//return nil, nil
		return nil, errors.New("missing 'dn:'"+lines[0])
	}
...
func (l *LDIF) parseAttrs(lines []string) (map[string][]string, error) {
	...
	for i := 0; i < len(lines); i++ {
		...
		if(SkipExcessiveZimbraTokens){
			if(attr == "zimbraLastLogonTimestamp"){
				continue
			}
			if(attr == "zimbraAuthTokens"){
				continue
			}
			if(attr == "entryCSN"){
				continue
...
// AllEntries returns all *ldap.Entries in the LDIF
func (l *LDIF) GetDN(needle string) (foundEntry bool, entry *ldap.Entry) {
	for _, entry := range l.Entries {
		if entry.Entry != nil {
			if entry.Entry.DN == needle{
				foundEntry = true
...
func (l *LDIF) GetAttributesByDN(DNEntry string) (foundEntry bool, entry []*ldap.EntryAttribute) {
	for _, entry := range l.Entries {
		if entry.Entry != nil {
			if entry.Entry.DN == DNEntry{
				foundEntry = true
				return true, entry.Entry.Attributes
			}
		}
	}
	return false, nil
